...
int register_user(aNick *nick, aChan *chaninfo, int parc, char **parv)
{
	...
	if(GetConf(CF_ONLYWEBREG) && GetConf(CF_WEBSERV) && !GetConf(CF_PREMIERE) && !IsAnAdmin(nick->user))
		return csreply(nick, "La commande est désactivée, utilisez l'interface sur %s", webaddr); 
	...
	/* on check ça avant tout parce que ça sert à rien de faire les verifs sinon*/
	...
	if(strcasecmp(parv[3], parv[2]))
		return csreply(nick, "Les deux emails fournis sont différents, vous avez surement commis une faute de frappe.");
	...
	else if(GetConf(CF_PREMIERE|CF_NOMAIL))
	{
		...
		if(GetConf(CF_PREMIERE))
		{
			...
			chaninfo = add_chan(bot.chan, "Salon d'aide à propos des services SDreams");
