 ...
 * $Id: aide.c,v 1.24 2006/03/16 18:32:34 bugs Exp $
 */
...
int aide(aNick *nick, aChan *chaninfo, int parc, char **parv)
{
	...
	if(!help || !help->buf)
        {
                Debug(W_WARN, "No help found for command %s, lang %s",
        ...
        if(parc > 1)
	{
		...
		csreply(nick, GetReply(nick, L_HELP1), parv[1], more,
			AdmCmd(cmdp) ? "4ADMIN" : HelpCmd(cmdp) ? "7HELPEUR" : ChanCmd(cmdp) ? "12CHAN" : "3USER", cmdp->level);
	}
	else {
	...
	list = help->buf[help->count-1];
        ...
        if(cmdp->flag & CMD_DISABLE) csreply(nick, "\2\0034Note:\2\3 %s", GetReply(nick, L_CMDDISABLE));
...
int help_load(Lang *lang)
{
        ...
        if(!(fp = fopen(buf, "r")))
        {
                Debug(W_MAX, "help: unable to open helpfile for lang %s", lang->langue);
                return -1; /* error */
        }
        ...
        if(i != CmdsCount)
                Debug(W_MAX, "help: Missing %d help sections for lang %s", CmdsCount - i, lang->langue);
