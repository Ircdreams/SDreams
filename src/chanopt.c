 ...
 * $Id: chanopt.c,v 1.27 2006/03/15 19:04:42 bugs Exp $
 */
...
int generic_chanopt(aNick *nick, aChan *chan, int parc, char **parv)
{
	...
	{/* nouveau parsage, plus couteux, mais plus fiable -- verif dès le début -Cesar*/
		...
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon DEFMODES <modes>", RealCmd("chanopt"));
		...
		if(!string2scmode(&tmp, parv[3], (k && k <= parc) ? parv[k] : NULL,
			...
			return csreply(nick, "Aucun mode valide défini.");
		if(!IsOper(nick) && tmp.modes & C_MOPERONLY) tmp.modes &= ~C_MOPERONLY;

		chan->defmodes = tmp;
		if(CJoined(chan)) modes_reset_default(chan);
		csreply(nick, GetReply(nick, L_DEFMODESARE), c, GetCModes(chan->defmodes));
	}

	else if(!strcasecmp(cmd, "welcome"))
	{
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon WELCOME <message|'none'>", RealCmd("chanopt"));

		if(!strcasecmp(parv[3], "none"))
		{
			*chan->welcome = '\0';
			DelCSetWelcome(chan);
			csreply(nick, GetReply(nick, L_OKDELETED), c);
		}
		else
		{
			parv2msgn(parc, parv, 3, chan->welcome, TOPICLEN);
			csreply(nick, GetReply(nick, L_WELCOMEIS), c, chan->welcome);
		}
	}
	...
	{
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon DEFTOPIC <topic>", RealCmd("chanopt"));

		if(CLockTopic(chan)) return csreply(nick, GetReply(nick, L_LOCKTOPIC), c);

		parv2msgn(parc, parv, 3, chan->deftopic, TOPICLEN);
		csreply(nick, GetReply(nick, L_DEFTOPICIS), c, chan->deftopic);
		cstopic(chan, chan->deftopic);
	}

	else if(!strcasecmp(cmd, "motd"))
	{
		if(parc < 3) return csreply(nick, "Syntaxe: %s #salon MOTD <motd>", RealCmd("chanopt"));
		...
		else
		{
			...
			csreply(nick, GetReply(nick, L_MOTDIS), c, chan->motd);
		}
	}
	...
	{
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon THEME <thème>", RealCmd("chanopt"));

		parv2msgn(parc, parv, 3, chan->description, DESCRIPTIONLEN);
		csreply(nick, GetReply(nick, L_DESCRIPTIONIS), c, chan->description);
	}

	else if(!strcasecmp(cmd, "chanurl"))
	{
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon CHANURL <url>", RealCmd("chanopt"));

		Strncpy(chan->url, parv[3], 80);
		csreply(nick, GetReply(nick, L_CHANURLIS), c, chan->url);
	}
	...
	else if(!strcasecmp(cmd, "banlevel"))
	{
		...
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon BANLEVEL <niveau>", RealCmd("chanopt"));

		if(!is_num(parv[3])) return csreply(nick, GetReply(nick, L_VALIDLEVEL));
		...
		if(chan->banlevel > l && !IsAdmin(nick->user))
			return csreply(nick, GetReply(nick, L_ACCESSTOOLOW), c, l, chan->banlevel);

		chan->banlevel = i;
		csreply(nick, GetReply(nick, L_BANLEVELIS), c, chan->banlevel);
	}

	else if(!strcasecmp(cmd, "bantype"))
	{
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon BANTYPE <n° du type>", RealCmd("chanopt"));

		if(!Strtoint(parv[3], &i, 1, 5))
			return csreply(nick, "Le type de ban doit être défini en précisant un nombre entre 1 et 5.");

		chan->bantype = i;
		csreply(nick, GetReply(nick, L_BANTYPEIS), chan->chan, GetBanType(chan), i);
	}

	else if(!strcasecmp(cmd, "strictop"))
		switch_option(nick, parc < 3 ? NULL : parv[3], cmd, c, &chan->flag, C_STRICTOP);
	
	else if(!strcasecmp(cmd, "autovoice")) {
                switch_option(nick, parc < 3 ? NULL : parv[3], cmd, c, &chan->flag, C_AUTOVOICE);
		if(CNoVoices(chan)) DelCNoVoices(chan); /* Suppression du novoice pour eviter les conflis */
	}
		...
		if(CAutoVoice(chan)) DelCAutoVoice(chan); /* Suppression de l'autovoice pour eviter les conflis */
	}

	else if(!strcasecmp(cmd, "nohalfops"))
		switch_option(nick, parc < 3 ? NULL : parv[3], cmd, c, &chan->flag, C_NOHALFOPS);
	...
	else if(!strcasecmp(cmd, "nopubcmd"))
		switch_option(nick, parc < 3 ? NULL : parv[3], cmd, c, &chan->flag, C_NOPUBCMD);

	else if(!strcasecmp(cmd, "chmodeslevel"))
	{
		int l = ChanLevelbyUserI(nick->user, chan);
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon CHMODESLEVEL <niveau>", RealCmd("chanopt"));
		...
		if(chan->cml > l && !IsAdmin(nick->user))
			return csreply(nick, GetReply(nick, L_ACCESSTOOLOW), c, l, chan->cml);

		chan->cml = i;
		csreply(nick, GetReply(nick, L_CHMODESLEVELIS), c, chan->cml);
	}
	...
	else if(!strcasecmp(cmd, "bantime"))
	{
		time_t timeout = 0;
		if(parc < 3)
			return csreply(nick, "Syntaxe: %s #salon BANTIME <durée>", RealCmd("chanopt"));

		if((timeout = convert_duration(*parv[3] == '%' ? ++parv[3] : parv[3])) < 0)
			return csreply(nick, "Durée incorrecte, le format est <XjXhXm> X étant le nombre respectif d'unités, ou 0 pour désactiver.");

		chan->bantime = timeout;
		if(timeout) csreply(nick, GetReply(nick, L_BANTIMEIS), c, chan->bantime, duration(timeout));
		else csreply(nick, GetReply(nick, L_OPTIONNOWOFF), cmd, c);
	}
	...
	else if(!strcasecmp(cmd, "autolimit"))
	{
		...
		{
			int linc = getoption("-limit_inc", parv, parc, 3, 1);
			int lmin = getoption("-limit_min", parv, parc, 3, 1);

			if((strcasecmp(parv[3], "on") && !lmin && !linc) || lmin < 0 || linc < 0)
				return csreply(nick, "Syntaxe: %s #salon AUTOLIMIT [ON|OFF] [-limit_inc <nombre>] [-limit_min <nombre>]", RealCmd("chanopt")); 
		...
		if(!CFLimit(chan)) csreply(nick, GetReply(nick, L_OPTIONNOWOFF), cmd, c);
		else csreply(nick, GetReply(nick, L_AUTOLIMITIS), c, chan->limit_min, chan->limit_inc);
...
int theme(aNick *nick, aChan *chan, int parc, char **parv)
{
...
{
	char *pparv[5] = { parv[0], parv[1], "locktopic", parv[2], NULL};
...
{
	char *pparv[5] = { parv[0], parv[1], "strictop", parv[2], NULL};
...
{
	char *pparv[5] = { parv[0], parv[1], "nobans", parv[2], NULL};
...
{
	char *pparv[5] = { parv[0], parv[1], "noops", parv[2], NULL};
...
{
	char *pparv[5] = { parv[0], parv[1], "setwelcome", parv[2], NULL};
...
{
	char *pparv[5] = { parv[0], parv[1], "banlevel", parv[2], NULL};
...
{
	char *pparv[5] = { parv[0], parv[1], "bantype", parv[2], NULL};
...
{
	char *pparv[5] = { parv[0], parv[1], "chanurl", parv[2], NULL};

	return generic_chanopt(nick, chan, parc + 1, pparv);
}

int autovoice(aNick *nick, aChan *chan, int parc, char **parv)
{
        char *pparv[5] = { parv[0], parv[1], "autovoice", parv[2], NULL};
