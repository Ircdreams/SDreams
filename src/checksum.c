...
static unsigned char PADDING5[64] = {
...
void MD5Final(UINT4 *digest, MD5_CTX *context)
{
	...
	MD5Update(context, PADDING5, padLen);

	/* Append length (before padding) */
	MD5Update(context, bits, 8);

	/* Store state in digest */
	memcpy(digest, context->state, sizeof context->state);
...
void MD2Update(MD2_CTX *context, unsigned char *input, unsigned int inputLen)
{
	...
	if(inputLen >= partLen) {
		memcpy(&context->buffer[index], input, partLen);
		MD2Transform((POINTER)context->state, (POINTER)context->checksum, context->buffer);

		for (i = partLen; i + 15 < inputLen; i += 16)
			MD2Transform((POINTER)context->state, (POINTER)context->checksum, &input[i]);
 ...
 */
void MD2Final(UINT4 *digest, MD2_CTX *context)
{
	unsigned int padLen;
