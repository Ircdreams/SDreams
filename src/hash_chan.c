 ...
 * $Id: hash_chan.c,v 1.18 2006/03/01 19:28:18 bugs Exp $
 */
...
aChan *add_chan(const char *chan, const char *description)
{
	...
	if(!strcasecmp(chan, bot.pchan)) return 0;
...
void del_chan(aChan *chan,int flag, const char *raison)
{
	...
	/* Suppression de tous les accès du salon
	 ...
	 tous les usernames vu qu'on a pas encore mis de lien entre chan et les
	 waitaccess
	 >> plus maintenant grâce au flag WAIT_ACCESS*/

	hash_delchan(chan); /* swap dans la hash*/

	putlog(LOG_CHANS, "UNREG %s - %s", chan->chan, raison);
	...
	SUSPEND_REMOVE(chan->suspend); 
        if(chan->timer) timer_remove(chan->timer); 
...
{ 
        if(!CFLimit(chan) && chan->timer) timer_remove(chan->timer), chan->timer = NULL; 
        else if(CFLimit(chan) && !chan->timer) 
                chan->timer = timer_add(cf_limit_update, TIMER_PERIODIC, callback_fl_update, chan, NULL); 
} 
...
void enforce_access_opts(aChan *c, aNick *n, anAccess *a, aJoin *j)
{
        ...
        if(!IsVoice(j) && ((AVoice(a) && !CNoVoices(c)) || CAutoVoice(c))) /* voice ? */
        {
                modes[i++] = 'v';
                DoVoice(j);
        }
	if(!IsHalfop(j) && AHalfop(a) && !CNoHalfops(c)) /* for halfop */
	{
		modes[i++] = 'h';
		DoHalfop(j);
	}
        ...
        {
                modes[i] = 0;
                csmode(c, MODE_OBVH, "+$ $ $ $", modes, n->numeric, i > 1 ? n->numeric : "", i > 2 ? n->numeric : "");
        }
}
